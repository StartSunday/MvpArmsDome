...
public class RxUtil {
    ...
    public static <T> ObservableTransformer<T, T> handleResult() {
        return upstream -> upstream.flatMap((Function<T, ObservableSource<T>>) t -> {
            if (t instanceof DataResponse && ((DataResponse) t).result != null && !((DataResponse) t).result.isSuccess()) {
                return Observable.error(
                        new ApiException(((DataResponse) t).result.getCode(), ((DataResponse) t).result.getMsg()));
            } else {
