...
public class ResponseErrorListenerImpl implements ResponseErrorListener {
    ...
    public void handleResponseError(Context context, Throwable t) {
        ...
        if (t instanceof UnknownHostException) {
            ...
            msg = ((ApiException) t).message;
            if (((ApiException) t).code == 909 && SPUtils.getInstance().getBoolean(Constant.HAS_LOGIN, false)) {//token失效
//                SPUtils.getInstance().clearUserInfo();
//                AppManager.getAppManager().killAll();
//                ARouter.getInstance().build(RouterHub.LOGIN_LOGIN_ACTIVITY).withCharSequence("to", "com.yice.card.mvp.ui.activity.MainActivity").navigation();
            }
